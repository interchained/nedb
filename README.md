...
## NEDB v2.2.30 — Production Stable

**Current stable: 2.2.30** — Cross-platform native wheels shipping `nedbd-v2` binary inside `pip install nedb-engine`. Linux + Windows wheels built on GitHub Actions; macOS arm64 + x86_64 wheels built on Codemagic (M2 Mac Minis). All four platforms publish from one `v*` tag.

NEDB v2 replaces the append-only log (AOF) with a **content-addressed Merkle DAG**. Every document version is an immutable, BLAKE2b-verified object. Nothing is ever overwritten. As of **v2.2.30**, restarts after the first open are **O(1) warm starts** (driven by a `MANIFEST` of `seq` + Merkle head), the **cold scan is deferred** so the daemon accepts connections immediately, and a new **`GET /events` SSE endpoint** streams scan progress + per-write events live.
...
# {"ok":true,"version":"2.2.30","service":"nedbd","engine":"dag","startup_ready":true,"encrypted":true}

# Tail the live event stream (new in v2.2.30)
...
**Production status:** [vision.interchained.org](https://vision.interchained.org) is live on v2.2.30 — **1,310,703 sequences** indexed in the Vision database, AES-256-GCM encrypted at rest, at block height **620,989**.
...
# Live event stream (new in v2.2.30) — SSE: scan progress, ready, per-write head
curl http://127.0.0.1:7070/events
```

### Startup modes (v2.2.30)
...
| `NEDBD_HOST` | `127.0.0.1` | Bind address. **v2.2.30** defaults to loopback (was `0.0.0.0`) — security hardening fix. Set explicitly to `0.0.0.0` to expose. |
...
**v2 DAG Rust server (v2.2.30, Intel iMac — 10k writes / 100k reads / 30k objects, AES-256-GCM on):**
...
| Repo | Description |
|---|---|
| [Eth-Interchained/nedb](https://github.com/Eth-Interchained/nedb) | Source — engine, Rust core, CI |
